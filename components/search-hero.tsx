            ...
            <SelectContent>
              {roles.map((roleItem) => (
                <SelectItem
                  key={roleItem.name}
                  className={cn(
                    " mt-2 pointer-events-auto cursor-pointer hover:!bg-[#0073cf] hover:!text-white ",
                    roleItem.role === role && "!bg-zinc-900/90 !text-white"
                  )}
                  value={roleItem.role}
                >
                  <button
      ...
      <div className="flex justify-center">
        <AnimatePresence mode="wait">
          {isVisible && (
            <motion.div
              className={cn(
                "md:p-10 p-3 flex gap-6 md:justify-start flex-wrap justify-center pointer-events-auto",
